...
function AddScorePisa(add)
{
	if (!CheckValid())
		return;

	scorePisa += +add;
	document.getElementById("proba-1").innerHTML = "Punctaj : " + scorePisa;
	WriteUserData();
}
...
{
	onValue(ref(database, "/" + clasa + "/" + name + "/pisa/punctaj"), (snapshot) =>
	{
		scorePisa = snapshot.val();
		document.getElementById("proba-1").innerHTML = "Punctaj : " + ReplaceNull(scorePisa);
	});
}
...
{
	set(ref(database, "/" + clasa + "/" + name + "/pisa"), {
		"punctaj": scorePisa
	});
}
...
document.getElementById("button-proba1-add").addEventListener("click", () => { AddScorePisa(scoreToAdd) });
document.getElementById("button-proba1-subtract").addEventListener("click", () => { AddScorePisa(-scoreToAdd) });